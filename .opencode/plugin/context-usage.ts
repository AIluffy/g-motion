    ...
    tool: {
      context_usage: tool({
        ...
        async execute(args, context) {
          ...
          const messages: SessionMessage[] = ((response as any)?.data ??
            response ??
            []) as SessionMessage[];
...
function extractText(parts: SessionMessagePart[]): string {
  return parts
    .filter(
      (part): part is { type: 'text'; text: string; synthetic?: boolean } => part.type === 'text',
    )
    .map((part) => part.text ?? '')
    .map((text) => text.trim())
    .filter(Boolean)
...
async function countTokens(content: string, model: TokenModel): Promise<number> {
  ...
  if (model.spec.kind === 'transformers') {
    ...
    try {
      ...
      return Array.isArray(encoding)
        ? encoding.length
        : (encoding?.length ?? Math.ceil(content.length / 4));
    } catch {
      return Math.ceil(content.length / 4);
    }
  }
